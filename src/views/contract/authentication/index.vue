            ...
            <div class="to-at" v-show="!startAt">
                ...
                <div v-else>
                    ...
                    <div class="form-text">
                        ...
                        <div class="group"><label>主体证件类型：</label>{{ rzInfo.certifyType | certifyTypeStr }}</div>
                        ...
                        <div class="group"><label>认证负责人姓名：</label>{{ rzInfo.legalPersonName }}</div>
                        <div class="group"><label>负责人证件类型：</label>{{ rzInfo.legalPersonCertifyType | certifyTypeStr }}</div>
                        ...
                        <div class="group"><label>认证状态：</label><span :class="'state state-'+rzInfo.status">{{ rzInfo.status | stateStr }}</span></div>
                        <div class="group">
                            <label></label>
                            <a class="btn" v-show="rzInfo.status == 1">变更信息</a>
                            <a class="btn" v-show="rzInfo.status == 9">再次认证</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        methods: {
            ...
            queryBaseInfo() {
                this.$http.post(this.$api.queryBaseInfo).then(res => {
                    if (res.code === 1000) {
                        this.isRz = res.data == '';
                        if(!this.isRz && res.data.status == 0){
                            this.startAtAction();
                        }
