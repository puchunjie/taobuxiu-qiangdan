<template>
  <div class="resource-container">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item,i) in list" :key="item.id">
              ...
              <td>{{ item.price }}</td>
              <td>{{ item.taxType | taxStr }}</td>
              <td>{{ item.locationName }}</td>
              <td>{{ item.storeHouseCount }}</td>
    ...
    <ModifyPrice v-model="modifyPriceShow" :apiUrl="$api.changePriceSpecialPrice" :batchIds="batchIds" :listData="editList" @on-ajax-success="getList"></ModifyPrice>
    <uploadExcel v-model="excelShow" :storeType="3" :uploadApi="$api.saveSpecialPriceByExcel" :historyApi="$api.findSpecialPriceExcel" @on-upload-success="getList"></uploadExcel>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.listLoad = true;
        this.$http.post(this.$api.findSpecialPricePage, this.apiParams).then(res => {
          if (res.code === 1000) {
      ...
      // 单个刷新
      refresh(item) {
        this.doSimple(item, this.$api.refreshSingleSpecialPrice, '刷新');
      },
      // 批量刷新
      batchRefresh() {
        this.doBatch(this.$api.refreshSpecialPrice, '刷新');
      },
      // 单个下架
      getOff(item) {
        this.doSimple(item, this.$api.removeSingleSpecialPrice, '下架');
      },
      // 批量下架
      batchGetoff() {
        this.doBatch(this.$api.removeSpecialPrice, '下架');
      },
      // 单个修改、上架
      edit(item, t, b) {
      ...
      // 单个删除
      del(item) {
        this.doSimple(item, this.$api.deleteSingleSpecialPrice, '删除');
      },
      //批量删除
      batchDelete() {
        this.doBatch(this.$api.deleteSpecialPrice, '删除');
      },
      // 批量操作公共方法
      doBatch(api, str) {
        if (this.batchCan) {
