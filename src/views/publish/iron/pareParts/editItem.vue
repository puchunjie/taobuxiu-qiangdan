...
<script>
    import fuzzySelector from './fuzzySelector.vue'
    ...
    export default {
        ...
        computed: {
            ...
            typeOfSelect() {
                return this.activeTargetRef.replace(/\d|id/g, "")
            },
            api() {
                let selectApi = '';
        ...
        methods: {
            ...
            // 填充数据
            fillItem(id, item) {
                let typeOfSelect = id.replace(/\d|id/g, "");
                switch (typeOfSelect) {
                    case 'city':
            ...
            // 输入框选择完毕后的验证
            validate(data) {
                let inputKey = data.id.replace(/\d|id/g, "");
                // 通过验证
                if (data.valide) {
